...
const directoryPath = path.join(__dirname, 'public/portfolio/pool-landscape-renovation-sod-round-hill'); 
...
(async () => {
  try {
    ...
    for (const file of files) {
      ...
      if (ext === '.webp' || ext === '.mov' || ext === '.mp4' || file.startsWith('.')) continue;
      ...
      try {
        if (ext === '.heic') {
          ...
          console.log(`✅ Convertido (HEIC): ${file} -> ${fileNameWithoutExt}.webp`);
        } else {
        ...
        console.error(`❌ Falló al procesar ${file} (No se eliminó el original):`, err.message);
      }
    }
